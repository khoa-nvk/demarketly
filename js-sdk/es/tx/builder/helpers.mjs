...
import { hash, salt } from "../../utils/crypto.mjs";
import { encode, decode } from "../../utils/encoder.mjs";
import { toBytes } from "../../utils/bytes.mjs";
import { ID_TAG_PREFIX, PREFIX_ID_TAG, NAME_BID_RANGES, NAME_FEE_BID_INCREMENT, NAME_BID_TIMEOUTS, NAME_MAX_LENGTH_FEE, POINTER_KEY_BY_PREFIX } from "./constants.mjs";
import { ceil } from "../../utils/bignumber.mjs";
import { TagNotFoundError, PrefixNotFoundError, InvalidNameError, IllegalBidFeeError, NoDefaultAensPointerError, ArgumentError } from "../../utils/errors.mjs";
/**
...
export function buildContractId(ownerId, nonce) {
    const ownerIdAndNonce = _Buffer.from([...decode(ownerId, 'ak'), ...toBytes(nonce)]);
...
export function oracleQueryId(senderId, nonce, oracleId) {
    function _int32(val) {
        ...
        return _Buffer.concat([_Buffer.alloc(32 - nonceBE.length), nonceBE]);
    }

    const b2bHash = hash(_Buffer.from([...decode(senderId, 'ak'), ..._int32(nonce), ...decode(oracleId, 'ok')]));
...
export function commitmentHash(name) {
    let salt = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : createSalt();
    ensureNameValid(name);
    return encode(hash(_Buffer.concat([_Buffer.from(name.toLowerCase()), formatSalt(salt)])), 'cm');
...
const addChecksum = input => {
    ...
    return _Buffer.concat([payload, getChecksum(payload)]);
};
...
export function writeId(hashId) {
    ...
    return _Buffer.from([...toBytes(idTag), ...decode(hashId, prefix)]);
}
/**
...
export function readId(buf) {
    const tag = _Buffer.from(buf).readUIntBE(0, 1);
...
export function computeBidFee(name, startFee) {
    let increment = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : NAME_FEE_BID_INCREMENT;
    ...
    return ceil(BigNumber(startFee !== null && startFee !== void 0 ? startFee : getMinimumNameFee(name)).times(BigNumber(NAME_FEE_BID_INCREMENT).plus(1)));
}
/**
...
export function computeAuctionEndBlock(name, claimHeight) {
    ...
    const h = length <= 4 && NAME_BID_TIMEOUTS[4] || length <= 8 && NAME_BID_TIMEOUTS[8] || length <= 12 && NAME_BID_TIMEOUTS[12] || NAME_BID_TIMEOUTS[13];
