...
import TxBuilder from "../tx/builder/index.mjs";
...
async function send(tx) {
  ...
  const signed = contractId ? await this.signUsingGA(tx, { ...opt,
    authFun
  }) : await this.signTransaction(tx, opt);
  return opt.innerTx ? {
    hash: TxBuilder.buildTxHash(signed),
    rawTx: signed
  } : this.sendTransaction(signed, opt);
}

async function signUsingGA(tx) {
  let {
    authData,
    authFun,
    ...options
...
async function spend(amount, recipientIdOrName, options) {
  ...
  return this.send(await this.spendTx({ ...opt,
...
async function transferFunds(fraction, recipientIdOrName, options) {
  ...
  } = TxBuilder.unpackTx(await this.spendTx({ ...opt,
    senderId,
    recipientId,
...
async function payForTransaction(transaction, options) {
  ...
  return this.send(await this.payingForTx({ ...opt,
