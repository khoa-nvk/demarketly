...
import { DisconnectedError, DuplicateNodeError, NodeNotFoundError, TypeError } from "../utils/errors.mjs";
/**
 * Node Pool Stamp
...
export default stampit({
  ...
  methods: {
     ...
     */
    getNodesInPool() {
      return Array.from(this.pool.entries()).map(_ref => {
        let [name, node] = _ref;
        return {
          name,
    ...
    validateNodes(nodes) {
      ...
      if (notObject) {
        throw new TypeError('Each node should have name (string), instance (object) ' + `fields, got ${JSON.stringify(wrongFields)} instead`);
      }

      const wrongInstanceFields = nodes.map(n => n.instance).find(i => typeof i.api !== 'object' || typeof i.genesisHash !== 'string' || typeof i.consensusProtocolVersion !== 'number');

      if (wrongInstanceFields) {
        throw new TypeError('Each node instance should have api (object), consensusProtocolVersion (number), ' + `genesisHash (string) fields, got ${JSON.stringify(wrongInstanceFields)} instead`);
      }
    }
