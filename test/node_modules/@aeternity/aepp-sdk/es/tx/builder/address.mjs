...
import { ArgumentError, PrefixNotFoundError, TagNotFoundError } from "../../utils/errors.mjs";
...
export function writeId(hashId) {
  ...
  return _Buffer.from([...toBytes(idTag), ...decode(hashId)]);
}
/**
...
export function readId(buf) {
  const idTag = _Buffer.from(buf).readUIntBE(0, 1);
