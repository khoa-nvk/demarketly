...
describe('DeMarketly contract', () => {
    ...
    it(`buy product with coupon fail because this coupon is all used`, async() => {
        const createProduct = await contract.methods.create_product(PRODUCT_ID_1, "name", 2000000000000000000, "description", "image", true, { onAccount: ownerAccount });
        const createCoupon = await contract.methods.create_coupon(PRODUCT_ID_1, COUPON_OFF_1AE, 0, COUPON_OFF_1AE_VALUE, { onAccount: ownerAccount })
            // const buyProduct = await contract.methods.buy_product(PRODUCT_ID_1, true, COUPON_OFF_1AE, { onAccount: nonOwnerAccount, amount: 2000000000000000000 });
        await assertNode.rejects(contract.methods.buy_product(PRODUCT_ID_1, true, COUPON_OFF_1AE, { onAccount: nonOwnerAccount }), (err) => {
            assert.include(err.message, "This coupon's allowed uses is 0");
            return true;
        });
    });

    it(`buy product with coupon fail because this coupon is not exist`, async() => {
        const createProduct = await contract.methods.create_product(PRODUCT_ID_1, "name", 2000000000000000000, "description", "image", true, { onAccount: ownerAccount });
        const createCoupon = await contract.methods.create_coupon(PRODUCT_ID_1, COUPON_OFF_1AE, 0, COUPON_OFF_1AE_VALUE, { onAccount: ownerAccount })
            // const buyProduct = await contract.methods.buy_product(PRODUCT_ID_1, true, COUPON_OFF_1AE, { onAccount: nonOwnerAccount, amount: 2000000000000000000 });
        await assertNode.rejects(contract.methods.buy_product(PRODUCT_ID_1, true, "randomcode", { onAccount: nonOwnerAccount }), (err) => {
            assert.include(err.message, "This coupon is not exist");
            return true;
        });
    });
